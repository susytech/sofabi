...
use heck::SnakeCase;
use syn::export::Span;
use sofabi::{Result, ResultExt, Contract, Param, ParamType};

const ERROR_MSG: &'static str = "`derive(SofabiContract)` failed";

#[proc_macro_derive(SofabiContract, attributes(sofabi_contract_options))]
pub fn sofabi_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
	let ast = syn::parse(input).expect(ERROR_MSG);
	let gen = impl_sofabi_derive(&ast).expect(ERROR_MSG);
	gen.into()
}

fn impl_sofabi_derive(ast: &syn::DeriveInput) -> Result<proc_macro2::TokenStream> {
	let options = get_options(&ast.attrs, "sofabi_contract_options")?;
...
fn to_syntax_string(param_type: &sofabi::ParamType) -> proc_macro2::TokenStream {
	match *param_type {
		ParamType::Address => quote! { sofabi::ParamType::Address },
		ParamType::Bytes => quote! { sofabi::ParamType::Bytes },
		ParamType::Int(x) => quote! { sofabi::ParamType::Int(#x) },
		ParamType::Uint(x) => quote! { sofabi::ParamType::Uint(#x) },
		ParamType::Bool => quote! { sofabi::ParamType::Bool },
		ParamType::String => quote! { sofabi::ParamType::String },
		ParamType::Array(ref param_type) => {
			let param_type_quote = to_syntax_string(param_type);
			quote! { sofabi::ParamType::Array(Box::new(#param_type_quote)) }
		},
		ParamType::FixedBytes(x) => quote! { sofabi::ParamType::FixedBytes(#x) },
		ParamType::FixedArray(ref param_type, ref x) => {
			let param_type_quote = to_syntax_string(param_type);
			quote! { sofabi::ParamType::FixedArray(Box::new(#param_type_quote), #x) }
		}
	}
}

fn to_sofabi_param_vec<'a, P: 'a>(params: P) -> proc_macro2::TokenStream
...
fn rust_type(input: &ParamType) -> proc_macro2::TokenStream {
	match *input {
		ParamType::Address => quote! { sofabi::Address },
		ParamType::Bytes => quote! { sofabi::Bytes },
		ParamType::FixedBytes(32) => quote! { sofabi::Hash },
		ParamType::FixedBytes(size) => quote! { [u8; #size] },
		ParamType::Int(_) => quote! { sofabi::Int },
		ParamType::Uint(_) => quote! { sofabi::Uint },
...
fn template_param_type(input: &ParamType, index: usize) -> proc_macro2::TokenStream {
	...
	match *input {
		ParamType::Address => quote! { #t_ident: Into<sofabi::Address> },
		ParamType::Bytes => quote! { #t_ident: Into<sofabi::Bytes> },
		ParamType::FixedBytes(32) => quote! { #t_ident: Into<sofabi::Hash> },
		ParamType::FixedBytes(size) => quote! { #t_ident: Into<[u8; #size]> },
		ParamType::Int(_) => quote! { #t_ident: Into<sofabi::Int> },
		ParamType::Uint(_) => quote! { #t_ident: Into<sofabi::Uint> },
...
fn to_token(name: &proc_macro2::TokenStream, kind: &ParamType) -> proc_macro2::TokenStream {
	match *kind {
		ParamType::Address => quote! { sofabi::Token::Address(#name) },
		ParamType::Bytes => quote! { sofabi::Token::Bytes(#name) },
		ParamType::FixedBytes(_) => quote! { sofabi::Token::FixedBytes(#name.to_vec()) },
		ParamType::Int(_) => quote! { sofabi::Token::Int(#name) },
		ParamType::Uint(_) => quote! { sofabi::Token::Uint(#name) },
		ParamType::Bool => quote! { sofabi::Token::Bool(#name) },
		ParamType::String => quote! { sofabi::Token::String(#name) },
		ParamType::Array(ref kind) => {
