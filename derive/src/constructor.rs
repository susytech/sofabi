...
impl<'a> From<&'a sofabi::Constructor> for Constructor {
	fn from(c: &'a sofabi::Constructor) -> Self {
		...
		let inputs_definitions = Some(quote! { code: sofabi::Bytes }).into_iter()
		...
		Constructor {
			...
			inputs_definitions,
			tokenize,
			recreate_inputs: to_sofabi_param_vec(&c.inputs),
		}
	}
}
...
impl Constructor {
	...
	pub fn generate(&self) -> TokenStream {
		...
		quote! {
			...
			pub fn constructor<#(#declarations),*>(#(#definitions),*) -> sofabi::Bytes {
				let c = sofabi::Constructor {
...
mod tests {
	...
	#[test]
	fn test_no_params() {
		let sofabi_constructor = sofabi::Constructor {
			inputs: vec![],
		};

		let c = Constructor::from(&sofabi_constructor);

		let expected = quote! {
			/// Encodes a call to contract's constructor.
			pub fn constructor<>(code: sofabi::Bytes) -> sofabi::Bytes {
				let c = sofabi::Constructor {
					inputs: vec![],
				};
				let tokens = vec![];
	...
	fn test_one_param() {
		...
		let expected = quote! {
			/// Encodes a call to contract's constructor.
			pub fn constructor<T0: Into<sofabi::Uint> >(code: sofabi::Bytes, foo: T0) -> sofabi::Bytes {
				let c = sofabi::Constructor {
				...
				let tokens = vec![sofabi::Token::Uint(foo.into())];
				c.encode_input(code, &tokens).expect(INTERNAL_ERR)
			}
		};
